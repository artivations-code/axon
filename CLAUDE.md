# Beyond the Sun Therapy — Website Content

Reference file for pages on the Beyond the Sun Therapy website. Copy below is
recorded **exactly as it appears in the approved design**. Do not reword,
re-punctuate, retitle, or "improve" it. If a change is needed, Katy authors it.

---

## Page: Recovery, Motherhood + Connection After Teen Placement

### Hero

**Heading**

> Recovery, Motherhood
> + Connection
> After Teen Placement

**Intro paragraph**

> Support for mothers navigating recovery, changed parenting roles, grief, and continued relationship after placing an adolescent or teen with another caregiver.

Hero imagery: sunlit window with sheer curtains, wooden desk with journals, a
notebook and pencil, a ceramic cup, and olive branches in a glass vase; water and
trees visible through the window. Soft olive brushstroke overlay on the left.

---

### Olive Statement Band

> Placing a child during adolescence can be a protective act of motherhood made under constrained circumstances.

Script line (handwritten style):

> Motherhood does not end when custody changes.

Olive/sage band, full width, centered text, cream brush-circle mark at right.

---

### Three Cards

#### 1. RECOVERY + EMOTIONAL SUSTAINABILITY

- Maintain recovery through emotionally charged family changes
- Prepare for grief, anniversaries, holidays, and milestones
- Build routines, supports, and boundaries that make steadiness more possible

LEARN MORE →

Card image: bed or daybed with cream linen throw and olive velvet pillow, sheer
curtain light, olive branches in a bronze vase.

#### 2. MOTHERHOOD AFTER PLACEMENT

- Understand what motherhood means after custody changes
- Grieve the loss of daily parenting and ordinary shared life
- Identify what remains within your role

LEARN MORE →

Card image: ceramic bowl of tea on a dark wood table with a pencil, papers, and
an olive sprig.

#### 3. CONNECTION WITH YOUR TEEN

- Prepare for calls, visits, messages, or renewed contact
- Respond to anger, silence, mixed feelings, or changing boundaries
- Build consistency without demanding closeness

LEARN MORE →

Card image: olive branch over a pebbled shoreline and calm water.

Each card sits under a small olive ring/circle icon, image above the heading,
bulleted list below, "LEARN MORE →" as the card link.

---

### Two Panels

#### RELATIONAL REPAIR WITHOUT FORCED RECONCILIATION

> Repair is not the same as obtaining access, receiving forgiveness, or restoring the former relationship.

> Support focuses on accountability, steadiness, and making repair attempts that do not require immediate forgiveness or closeness.

LEARN MORE →

#### WORKING WITH THE CURRENT CAREGIVING FAMILY

> When appropriate and agreed upon, support may include collaborative work with relatives, guardians, foster caregivers, or adoptive parents.

> We can address communication breakdowns, contact expectations, boundaries, and how adults speak about one another.

LEARN MORE →

Cream/paper-textured section, two columns divided by a thin vertical olive rule.
Torn-paper collage at far left, faint olive leaf watermark at far right.

---

### Closing CTA Band

**Heading**

> START WITH A FREE 15-MINUTE PHONE CONSULTATION

**Supporting line**

> A brief call to talk about what's happening and explore whether support together makes sense.

**Button**

> SCHEDULE A CONSULTATION

Olive band, speech-bubble icon at left, cream outlined button at right.

---

### Page Notes

- Section order: Hero → Olive Statement Band → Three Cards → Two Panels → Closing CTA Band.
- Palette: olive/sage, warm cream, soft off-white, muted bronze.
- Type: serif headings, sans-serif body, script accent for the statement line only.
- Every card and panel ends in "LEARN MORE →"; the page ends in one CTA button.
